...
then
	...
	echo NO_PROXY=169.254.169.254,169.254.170.2,/var/run/docker.dock >> /etc/ecs/ecs.config
...
# Enable docker host networking mode if desired
if [ $DOCKER_NETWORK_MODE = "host" ]
then
	sudo sed -i -e "s|^\(OPTIONS=\".*\)\"$|\1 --bridge=none --ip-forward=false --ip-masq=false --ip-tables=false\"|" /etc/sysconfig/docker
fi
